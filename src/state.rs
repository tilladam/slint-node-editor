    ...
    N: NodeGeometry + Copy,
{
    ...
    /// Compute bezier path using pure world coordinates.
    ///
    /// Container is at (pan-x, pan-y) with transform-scale: zoom.
    /// Everything inside the container uses pure world coordinates.
    /// The zoom and pan parameters are kept for API compatibility but ignored.
